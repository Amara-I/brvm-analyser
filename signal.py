...
def get_brvm_tickers() -> List[str]:
    """
    Retourne la liste des codes/actions BRVM les plus actifs.
    Ces codes peuvent être mis à jour selon les listings BRVM.
    """
    # Liste des principales actions BRVM (à adapter selon période)
    return [
...
def fetch_brvm_data() -> Dict:
    """
    Extrait les données de marché depuis BRVM.org et retourne un dictionnaire.
   -met en cache les données pour éviter les requêtes excessives.
    """
    cache_age = None
    if os.path.exists(DATA_FILE):
        file_age = datetime.now() - datetime.fromtimestamp(os.path.getmtime(DATA_FILE))
        cache_age = file_age.total_seconds() / 60  # en minutes
    
    # Recharger seulement si données de plus de 5 minutes
    if cache_age is not None and cache_age < 5:
        with open(DATA_FILE, 'r') as f:
            return json.load(f)
    
    # Extraction des données BRVM (simulation pour les tests)
    # En production, utiliserrequests pour requêter BRVM.org
...
def simulate_brvm_data() -> Dict:
    ...
    À remplacer par la vraie extraction BRVM en production.
    """
    np.random.seed(42)
...
def calculate_bollinger_bands(prices: pd.Series, period: int = 20, std_dev: float = 2) -> Tuple[pd.Series, pd.Series, pd.Series]:
    """Calcule les Bandes de Bollinger (upper, middle, lower)."""
...
# ============================================================================
# ANALYSE FONDAMENTALE (Simulée)
# ============================================================================

def get_fundamental_data(ticker: str) -> Dict:
    """
    Retourne les données fondamentales simulées pour une action.
    En production, extraire depuis SikaFinance ou autres sources.
    """
    np.random.seed(hash(ticker) % 1000)
    
    return {
        "market_cap": np.random.uniform(10, 500) * 1e9,  # FCFA
...
def calculate_technical_signals(ticker: str, historical_data: pd.DataFrame) -> Dict:
    """
    Calcule tous les indicateurs techniques et retourne un dictionnaire de signaux.
    """
    ...
    signals = {
        ...
        "rsi_signal": "survente" if last_rsi < 30 else ("surachat" if last_rsi > 70 else "neutre"),
        ...
        "stoch_signal": "survente" if last_stoch_k < 20 else ("surachat" if last_stoch_k > 80 else "neutre"),
        
        # ATR (volatilité)
        "atr": round(atr.iloc[-1], 2) if len(atr) > 0 else 0,
        
        # Tendance
        "trend": "haussière" if last_close > sma_20.iloc[-1] if len(sma_20) > 0 else "incertaine" else "baissière"
    }
...
def calculate_fundamental_score(ticker: str) -> float:
    """
    Calcule un score fondamental de 0-100 basé sur les fondamentaux.
    """
    fund_data = get_fundamental_data(ticker)
    
    score = 50  # Score de base
    
    # P/E Ratio (plus bas = mieux si > 0)
    ...
    # Dividend Yield (plus haut = mieux)
...
def calculate_overall_score(ticker: str, technical_signals: Dict, fundamental_score: float) -> Tuple[int, str]:
    """
    Calcule le score global (0-100) combinant technique et fondamental.
    """
    tech_score = 50
    ...
    if "rsi" in technical_signals:
        ...
        if rsi_val < 30:
            tech_score += 10  # Survente = opportunity
        elif rsi_val > 70:
            tech_score -= 10  # Surachat = risque
    ...
    # Limiter le score technique entre 0 et 100
...
def generate_signals_summary(buy_signals: List[Dict], sell_signals: List[Dict]) -> str:
    """
    Génère un résumé textuel des signaux pour les alertes email.
    """
    summary = """
...
def analyze_market(force_refresh: bool = False) -> Dict:
    """
    Fonction principale: analyse complète du marché BRVM.
    Retourne un dictionnaire avec tous les signaux triés.
    """
    if force_refresh or not os.path.exists(SIGNALS_FILE):
        ...
        for ticker in get_brvm_tickers():
            if ticker in market_data:
                ...
                # Générer données historiques simulées (30 jours)
...
def generate_historical_data(ticker: str, current_price: float, days: int = 30) -> pd.DataFrame:
    """
    Génère des données historiques simulées pour les calculs d'indicateurs.
    En production, remplacer par de vraies données historiques.
    """
...
def get_signal_for_ticker(ticker: str) -> Optional[Dict]:
    """
    Retourne les signaux pour un ticker spécifique.
    """
...
def get_top_signals(n: int = 5, recommendation: str = "ACHAT") -> List[Dict]:
    """
    Retourne les top N signaux pour une recommandation donnée.
    """
    signals_data = analyze_market()
...
if __name__ == "__main__":
    ...
    print(f"\n📊 Analyse terminée: {len(result['all_signals'])} actions analysées")
    print("\n🏆 TOP 5 ACHAT:")
